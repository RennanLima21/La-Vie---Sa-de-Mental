...
const atendimentosController = {
  ...
  async listarAtendimentoID(req, res) {
    try {
      ...
      if (!listarUmAtendimento) return res.status(404).json("id não encontrado");
  ...
  async deletarAtendimento(req, res) {
    try {
      const { id } = req.params;
      const atendimentoToDelete = await Atendimentos.findByPk(id, {
      });
      if (!atendimentoToDelete) return res.status(404).json("id não encontrado");
