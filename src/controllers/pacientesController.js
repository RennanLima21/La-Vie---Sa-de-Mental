...
const pacientesController = {
  ...
  async deletarPaciente(req, res) {
    try {
      const { id } = req.params;
      const pacienteToDelete = await Pacientes.findByPk(id, {
        
      });
      if (!pacienteToDelete) return res.status(404).json("id não encontrado");
  ...
  async atualizarPaciente(req, res) {
    try {
      const { id } = req.params;
      const { nome, email, idade } = req.body;
      const pacienteToUpdate = await Pacientes.findByPk(id, {
        
      });
      if (!pacienteToUpdate) return res.status(404).json("id não encontrado");
